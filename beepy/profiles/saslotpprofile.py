	...
	def doProcessing(self):
		...
		if theframe:
			...
			if status:
				...
				if blob:
					self.log.logmsg(logging.LOG_DEBUG, "blob: %s" % blob)
...
#							raise NotImplementedError('test')
						...
						self.log.logmsg(logging.LOG_DEBUG, "Finished responding to challenge")
...
#			passphrase = self.getPassphraseFromUser()
	...
	def authenticate(self, blob):
		...
		return self.generator.authenticate(self.algo, self.authid, passhash)
	...
	def parseResponse(self, data):
		...
		data = data[4:]
		self.log.logmsg(logging.LOG_DEBUG, 'parsing response: %s' % data)
		data = self.generator.convertHexToBytes(data)
		self.log.logmsg(logging.LOG_DEBUG, 'converted to: %s' % data)
		return data
	...
	def sendAuth(self, passphrase, authentid, authid=None):
		...
		if authid:
			...
			self.authid = authentid

		data = self.authid + '\000' + self.authentid
		data = self.encodeBlob(data)
		return self.channel.sendMessage(data)

	def splitAuth(self, data):
		"""splitAuth() undoes the concatenation of authid and authentid
...
class OTPGenerator(OTPdbase):
	...
	def createOTP(self, username, algo, seed, passphrase, sequence):
		"""generateOTP() should be used if you want to get
	...
	def validatePassphrase(self, passphrase):
		...
		if len(passphrase) > 63:
			self.log.logmsg(logging.LOG_WARN, "Passphrase longer than recommended length of 63 characters.")
	...
	def generateHash(self, algo, data):
		if algo == 'md5':
#			self.log.logmsg(logging.LOG_DEBUG, "Generating MD5 hash of: %s" % data)
	...
	def authenticate(self, algo, username, bytestring):
		...
		self.log.logmsg(logging.LOG_DEBUG, 'comparing dbEntry: %s to %s' % (dbEntry.passphrasehash, mypasshash) )
