# $Id: tcpsession.py,v 1.7 2003/01/30 09:24:29 jpwarren Exp $
# $Revision: 1.7 $
#
...
class TCPInitiator(session.Initiator, util.isMonitored):
    ...
    def _stateINIT(self):
        ...
        try:
            self.log.logmsg(logging.LOG_INFO, "Initiating connection to %s[%s]..." % (self.server_address) )
            ...
            self.log.logmsg(logging.LOG_ERR, "Connection to remote host failed: %s: %s" % ( e.__class__, e) )
            self.transition('error')
            return
...
class TCPListenerManager(session.ListenerManager, util.LoopingThread):
    ...
    def __init__(self, log, profileDict, address, daemon=0, max_connect=5, max_concurrent=50, accept_timeout=1, read_timeout=1, name=None ):
    ...
    def _stateINIT(self):
        ...
        try:
            self.start_socket()
            self.log.logmsg(logging.LOG_NOTICE, "ListenerManager started on %s[%s]." % self.address )
            self.transition('ok')
            return
...
class TCPInitiatorManager(session.InitiatorManager, util.LoopingThread):
    ...
    def __init__(self, log, profileDict, daemon=0, timeout=1, name=None):
        util.LoopingThread.__init__(self, name=name)
    ...
    def connectInitiator(self, address, profileDict=None):
        if not profileDict:
            profileDict = self.profileDict
        client = TCPInitiator(self.log, profileDict, address, self)
